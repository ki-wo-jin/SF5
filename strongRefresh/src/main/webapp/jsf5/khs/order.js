document.getElementById('payment').addEventListener('click', async function() {
    ...
    try {
        ...
        if (result.retCode === 'OK') {
            ...
            // 장바구니 항목 삭제
            await fetch('removeSelectedCartItems.do', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ orderCode: result.orderCode, id: selectedItems[0].id })
            });

            // 주문 상태 업데이트
            await fetch('updateOrderStatus.do', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ orderCode: result.orderCode, status: '주문완료' })
            });
